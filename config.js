...
function initConfig({ dir, compileScript = 'npm run compile', buildScript = 'npm run build', customScript }) {
    const homedir = os.homedir();
    const configPath = path.join(homedir, '.local-package-cli-config.json');

    fs.ensureFileSync(configPath);
    fs.writeJsonSync(configPath, { dir, compileScript, buildScript, customScript, inited: true });
}
