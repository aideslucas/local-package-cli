...
const setHandler = ({ dir, compileScript, buildScript, customScript }) =>
...
const copyBuilder = (command) => {
  ...
  command.positional("watch", {
    describe: "watch files and rerun copy on changes",
  });
};
...
const installBuilder = (command) => {
  command.positional("packageName", {
    describe:
      "the name of the package you would want to install (as described in package.json)",
  });
  command.positional("compile", {
    describe: "run compile script on package before installing",
  });
  command.positional("build", {
    describe: "run build script on package before installing",
  });
  command.positional("custom", {
    describe: "run custom script on package before installing",
  });
};
...
const installHandler = ({ packageName, compile, build, custom }) => {
  const config = getConfig();
  if (config && config.inited) {
    out.info(`installing package ${packageName}`);
...
yargs(hideBin(process.argv))
  ...
  .command("getConfig", "gets the config values", () => {}, getHandler)
  .command(
